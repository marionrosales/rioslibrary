				...
				<table datatable="ng" dt-options="vm.dtOptions" class="table table-bordered table-striped">
					...
					<tbody>
						<tr ng-repeat="book in bookData">
							...
							<td>{{book.genre}}</td>
							<td>{{book.section}}</td>
							<td><button type="button" ng-click="borrow(book.id)" class="btn btn-success btn-xs">Return</button></td>
						</tr>
					</tbody>
				</table>
			</div>
...
app.controller('crudController', function($scope, $http){
	...
	$scope.fetchData = function(){
		...
		$http.get('fetch_data_borrowed.php?id='+'<?php echo $_GET['id']?>').success(function(data){
			$scope.bookData = data;
		});
	};
	...
	$scope.borrow = function(id){
		...
		$http({
			method:"POST",
			url:"insert.php",
			data:{'id':id, 'user_id' : '','stat': stat, 'action':'UpdateStat'}
		}).success(function(data){
			$scope.hidden_id = id;
			$scope.success = true;
			$scope.error = false;
